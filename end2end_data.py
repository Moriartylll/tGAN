...
class BasicDataset():
    def __init__(self, seg_map_seqs_dir,images_seqs_dir,transforms_in=None,transforms_out=None,n_frames_total=20):
        self.seg_map_seqs_dir = seg_map_seqs_dir
        self.images_seqs_dir = images_seqs_dir
        self.n_frames_G=2
        self.transforms_in=transforms_in
        self.transforms_out = transforms_out
    ...
    def __getitem__(self, seq_idx):
        ...
        B_list_new, A_list_new = self.preprocess(B_list, A_list, self.transforms_in)
        C_list_new, _ = self.preprocess(B_list, A_list, self.transforms_out)
        A,B,C=None,None,None
        for Bi,Ai,Ci in zip(B_list_new, A_list_new,C_list_new):
            A = concat_frame(A, Ai, n_frames_total)
            B = concat_frame(B, Bi, n_frames_total)
            C = concat_frame(C, Ci, n_frames_total)

        return_list = {'A': A, 'B': B,'C': C}
        return return_list
    ...
    def __len__(self):
        return len(self.images_seqs_dir)
